...
- **Current baseline:** [`@agent-workflow/engine@0.1.4`](https://www.npmjs.com/package/@agent-workflow/engine) implements the profile in [`docs/engine-profile.md`](docs/engine-profile.md): POC core nodes, **R2** orchestration (`parallel`, `wait`, `set_state`), and **R3** native delegation/composition (`agent_delegate`, `subworkflow`) with in-process mock A2A. Host-mediated and engine-direct activity execution per [ADR-0003](docs/architecture/adr/ADR-0003-engine-direct-mcp-activity-execution.md). Schema validation and conformance harness (`npm run conformance`) are green on `main`.
