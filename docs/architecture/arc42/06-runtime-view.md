...
**Parity harness (R3 runway):** `conformance/vectors/parity/` runs table-driven scenarios against the application port and MCP handlers in-process; normalized snapshots must match per step ([`../arc42-assets/contracts/integration-parity-matrix.md`](../arc42-assets/contracts/integration-parity-matrix.md)). R3 delegate/subworkflow correlation rows are stubbed `pending` until features [#6](https://github.com/benvdbergh/workflows/issues/6)/[#7](https://github.com/benvdbergh/workflows/issues/7) land.
