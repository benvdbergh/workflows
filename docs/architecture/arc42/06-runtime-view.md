...
**Parity harness (R3 runway):** `conformance/vectors/parity/` runs table-driven scenarios against the application port and MCP handlers in-process; normalized snapshots must match per step ([`../arc42-assets/contracts/integration-parity-matrix.md`](../arc42-assets/contracts/integration-parity-matrix.md)). **`subworkflow`** runtime is in profile ([#7](https://github.com/benvdbergh/workflows/issues/7)); parity row `parity.r3.subworkflow_status_correlation` stays `pending` until [#8](https://github.com/benvdbergh/workflows/issues/8) exposes child ids on `workflow_status`. Delegate correlation pending [#6](https://github.com/benvdbergh/workflows/issues/6)/#8.
