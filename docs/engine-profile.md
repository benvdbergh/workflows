...
|----------|---------------------------|
| [Workflow Definition Schema](RFC/rfc-03-workflow-definition-schema.md) | §3.1–§3.8 (representation, top-level shape, jq, state/reducers, nodes, edges, node types listed below, retry/timeout) |
| [Execution Model](RFC/rfc-04-execution-model.md) | §4.2–§4.6, §4.8 (phases, command/event subsets below, state updates and reducers, interrupt/resume) |

---

## 1. Workflow document surface (in scope)

Per [RFC-03 §3.2](RFC/rfc-03-workflow-definition-schema.md#32-top-level-document-structure), workflow documents in this profile **MUST** include:
...
These discriminators **MUST** be supported by the workflow schema bundle and honored by the first engine milestone (naming matches [RFC-03 §3.7](RFC/rfc-03-workflow-definition-schema.md#37-node-types-normative)):
...
| `wait` | `config.kind`: `duration` (requires `duration_ms` or parseable `duration` string), `until` (ISO-8601 `until` timestamp), or `signal` (requires host `workflow_signal`; emits `StartSignalWait` / `SignalWaitStarted` and resumes on `DeliverSignal` / `SignalReceived`; signal `payload` keys merge into workflow state via `state_schema` reducers). |
...
Common node fields [RFC-03 §3.5](RFC/rfc-03-workflow-definition-schema.md#35-node-object-common-fields): `id`, `type`, optional `config`, `retry`, `timeout`, `metadata` — all **in scope** where applicable.
...
Per [RFC-03 §3.6](RFC/rfc-03-workflow-definition-schema.md#36-edges):
...
Per [RFC-03 §3.3](RFC/rfc-03-workflow-definition-schema.md#33-expression-language-jq):
...
Per [RFC-03 §3.4](RFC/rfc-03-workflow-definition-schema.md#34-state-schema-and-reducers) and [RFC-04 §4.6](RFC/rfc-04-execution-model.md#46-state-updates-and-reducers), this profile **MUST** support these `state_schema` property annotations when merging node outputs into state:
...
After reducer application, engines **SHOULD** validate state against `state_schema` when validation is enabled ([RFC-04 §4.6](RFC/rfc-04-execution-model.md#46-state-updates-and-reducers)).
...
The full taxonomies are [RFC-04 §4.4](RFC/rfc-04-execution-model.md#44-command-taxonomy-normative) and [§4.5](RFC/rfc-04-execution-model.md#45-event-taxonomy-normative).
...
- `EmitSignal` (RFC alias; reference engine uses `DeliverSignal`)
...
- `CheckpointWritten` — emitted when checkpointing is not `disabled`; payload `policy` is `after_each_node` or `every_n_nodes` (with `intervalNodes` when interval policy is used). Checkpoints taken **inside a parallel branch** (per-branch walk before the join target) include **`parallelSpan`**: `{ parallelNodeId, joinTargetId, branchName, branchEntryNodeId }` so readers can correlate inline `stateRef` with fork/join context ([RFC-04 §4.10](RFC/rfc-04-execution-model.md#410-checkpointing)).

Lifecycle phases [RFC-04 §4.2](RFC/rfc-04-execution-model.md#42-phases) **MUST** be respected at a high level: validate → start → walk graph → complete or fail; interrupt transitions per [§4.8](RFC/rfc-04-execution-model.md#48-interrupt-and-resume-protocol). Deterministic replay [§4.3](RFC/rfc-04-execution-model.md#43-deterministic-replay) is **normative for the protocol** but **MAY** land in a later milestone than the first runnable scheduler; this document still lists the command/event shapes the profile definition is intended to align with.
...
**Profile model (core vs optional):** [profile-model.md](profile-model.md), [jq-conformance-subset.md](jq-conformance-subset.md), [migration-alpha-to-ga.md](migration-alpha-to-ga.md).
